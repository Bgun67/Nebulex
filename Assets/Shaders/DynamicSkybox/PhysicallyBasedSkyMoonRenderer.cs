...
class PhysicallyBasedSkyMoonRenderer : SkyRenderer
{
    ...
    class PrecomputationData
    {
        ...
        {
            var table = RTHandles.Alloc((int)PbrSkyMoonConfig.GroundIrradianceTableSize, 1,
        ...
        RTHandle AllocateInScatteredRadianceTable(int index)
        {
            ...
            var table = RTHandles.Alloc((int)PbrSkyMoonConfig.InScatteredRadianceTableSizeX,
                                        (int)PbrSkyMoonConfig.InScatteredRadianceTableSizeY,
                                        (int)PbrSkyMoonConfig.InScatteredRadianceTableSizeZ *
                                        (int)PbrSkyMoonConfig.InScatteredRadianceTableSizeW,
            ...
            using (new ProfilingScope(cmd, ProfilingSampler.Get(HDProfileId.InScatteredRadiancePrecomputation)))//
            {
                int order = m_LastPrecomputedBounce + 1;
                {
                    ...
                    for (int i = 0; i < numPasses; i++)
                    {
                        ...
                        cmd.DispatchCompute(s_InScatteredRadiancePrecomputationCS, pass, (int)PbrSkyMoonConfig.InScatteredRadianceTableSizeX / 4,
                                                                                         (int)PbrSkyMoonConfig.InScatteredRadianceTableSizeY / 4,
                                                                                         (int)PbrSkyMoonConfig.InScatteredRadianceTableSizeZ / 4 *
                                                                                         (int)PbrSkyMoonConfig.InScatteredRadianceTableSizeW);
                    }
                    ...
                    cmd.DispatchCompute(s_GroundIrradiancePrecomputationCS, firstPass, (int)PbrSkyMoonConfig.GroundIrradianceTableSize / 64, 1, 1);
                }
            }
        }
        ...
        public bool Update(BuiltinSkyParameters builtinParams, PhysicallyBasedSkyMoon pbrSky)
        {
    ...
    ShaderVariablesPhysicallyBasedSkyMoon m_ConstantBuffer;
    int m_ShaderVariablesPhysicallyBasedSkyID = Shader.PropertyToID("ShaderVariablesPhysicallyBasedSkyMoon");//ShaderVariablesPhysicallyBasedSky

    static GraphicsFormat s_ColorFormat = GraphicsFormat.R16G16B16A16_SFloat;


    public PhysicallyBasedSkyMoonRenderer()
    {
    }
    ...
    public override void Build()
    {
        ...
        //HDRenderPipeline.currentAsset;
        HDRenderPipelineAsset hdrpResources = (HDRenderPipelineAsset)GraphicsSettings.renderPipelineAsset;
        
        
        

        // Shaders
        s_GroundIrradiancePrecomputationCS = (ComputeShader)Resources.Load("GroundIrradiancePrecomputation");//hdrpResources.shaders.groundIrradiancePrecomputationCS;
        s_InScatteredRadiancePrecomputationCS = (ComputeShader)Resources.Load("InScatteredRadiancePrecomputation");//hdrpResources.shaders.inScatteredRadiancePrecomputationCS;
        s_PbrSkyMaterialProperties = new MaterialPropertyBlock();

        //m_PbrSkyMaterial = CoreUtils.CreateEngineMaterial("Skybox/PbrSkyMoon");//hdrpResources.shaders.physicallyBasedSkyPS);
        Shader skyMoonShader = (Shader)Resources.Load("PhysicallyBasedSkyMoon");
        m_PbrSkyMaterial = CoreUtils.CreateEngineMaterial(skyMoonShader);
    ...
    {
        var pbrSky = builtinParams.skySettings as PhysicallyBasedSkyMoon;
    ...
    public override void RenderSky(BuiltinSkyParameters builtinParams, bool renderForCubemap, bool renderSunDisk)
    {
        ...
        bool simpleEarthMode = pbrSky.type.value == PhysicallyBasedSkyMoonModel.EarthSimple;
        ...
        int hasGroundSpecTexture = 0;

        if (pbrSky.groundSpecTexture.value != null && !simpleEarthMode)
        {
            hasGroundSpecTexture = 1;
            s_PbrSkyMaterialProperties.SetTexture("_GroundSpecTexture", pbrSky.groundSpecTexture.value);
        }
        s_PbrSkyMaterialProperties.SetInt(HDShaderIDs._HasGroundSpecTexture, hasGroundSpecTexture);
        ...
        int hasCloudTexture = 0;

        if (pbrSky.cloudTexture.value != null && !simpleEarthMode)
        {
            hasCloudTexture = 1;
            s_PbrSkyMaterialProperties.SetTexture(HDShaderIDs._CloudTexture, pbrSky.cloudTexture.value);
        }
        s_PbrSkyMaterialProperties.SetInt(HDShaderIDs._HasCloudTexture, hasCloudTexture);
    ...
    static class HDShaderIDs
    {
        ...
        public static readonly int _HasGroundSpecTexture            = Shader.PropertyToID("_HasGroundSpecTexture");
        public static readonly int _GroundSpecTexture               = Shader.PropertyToID("_GroundSpecTexture");
        ...
        public static readonly int _HasCloudTexture          = Shader.PropertyToID("_HasCloudTexture");
