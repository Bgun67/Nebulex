...
//#include "Packages/com.unity.render-pipelines.high-definition/Runtime/Sky/PhysicallyBasedSky/ShaderVariablesPhysicallyBasedSkyMoon.cs.hlsl"
#include "ShaderVariablesPhysicallyBasedSkyMoon.cs.hlsl"
...
{
    const uint zTexSize = PBRSKYMOONCONFIG_IN_SCATTERED_RADIANCE_TABLE_SIZE_Z;
    const uint zTexCnt  = PBRSKYMOONCONFIG_IN_SCATTERED_RADIANCE_TABLE_SIZE_W;

    float cosChi = -NdotV;

    float u = MapAerialPerspective(cosChi, height, rcp(PBRSKYMOONCONFIG_IN_SCATTERED_RADIANCE_TABLE_SIZE_X)).x;
    float v = MapAerialPerspective(cosChi, height, rcp(PBRSKYMOONCONFIG_IN_SCATTERED_RADIANCE_TABLE_SIZE_X)).y;
