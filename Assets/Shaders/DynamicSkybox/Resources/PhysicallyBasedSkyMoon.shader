Shader "Skybox/PbrSkyMoon"
{
    ...
    #include "../PhysicallyBasedSkyMoonCommon.hlsl"
    ...
    int _HasGroundSpecTexture;    // bool...
    int _HasGroundEmissionTexture;  // bool...
    int _HasCloudTexture;  // bool...
    ...
    TEXTURECUBE(_GroundSpecTexture);
    TEXTURECUBE(_GroundEmissionTexture);
    TEXTURECUBE(_CloudTexture);
    ...
    float4 RenderSky(Varyings input)
    {
        ...
        if (renderSunDisk)
        {
            ...
            for (uint i = 0; i < _DirectionalLightCount; i++)
            {
                ...
                //Something is wrong here, Michael
                //I've negated all three, not sure why this works
        ...
        if (rayIntersectsAtmosphere && !lookAboveHorizon) // See the ground?
        {
            ...
            if (tGround < tFrag)
            {
                ...
                if (_HasGroundSpecTexture)
                {
                    spec = SAMPLE_TEXTURECUBE(_GroundSpecTexture, s_trilinear_clamp_sampler, mul(gN, (float3x3)_PlanetRotation)).rgb;
                }
                float3 cloudTransparency = 0.0;
                if (_HasCloudTexture)
                {
                    // V points towards the camera.
                    float3 ts = SAMPLE_TEXTURECUBE(_CloudTexture, s_trilinear_clamp_sampler, mul(gN, (float3x3)_PlanetRotation)).rgb;
                    cloudTransparency = ts;
                }
                ...
                for (uint i = 0; i < _DirectionalLightCount; i++)
                {
                    ...
                    spec = lerp(spec, float3(0,0,0), 1.0*(cloudTransparency));
                    ...
                    radiance += lerp(gBrdf, float3(1,1,1), 1.0*(cloudTransparency)) * (lerp(irradiance, 1.0*smoothstep(0.4,0.8,dot(gN, L)), spec)*intensity);
                    radiance += 50.0*smoothstep(0.7,1.0,dot(gN, L)) * ((1-spec)) * intensity * gBrdf;
                    //radiance = SampleGroundIrradianceTexture(0.5) * 1000.0;
                    //radiance = smoothstep(0.3,0.7,dot(gN, L))*100.0;
                }
            }
        }

        /*if (true) // See the ground?
        {
            float tGround = tEntry + IntersectSphere(_MoonRadius, cosChi, r).x;
            ...
            if (tGround < tFrag)
            {
                ...
                for (uint i = 0; i < _DirectionalLightCount; i++)
                {
                    ...
                    //radiance += gBrdf * (irradiance * intensity); // Scale from unit intensity to light's intensity
                    radiance += gBrdf * (lerp(irradiance, irradiance+smoothstep(0.4,0.6,dot(gN, L)), spec)*intensity);
                    //radiance = SampleGroundIrradianceTexture(0.5) * 1000.0;
                    //radiance = smoothstep(0.3,0.7,dot(gN, L))*100.0;
                }
            }
