...
function Awake () {
	if(testing){
		var sr = new StreamReader(System.IO.Path.Combine(Application.streamingAssetsPath, "Player Settings.txt"));
...
function DisplayHostMenu () {
    ...
    if (GUI.Button (Rect (325, 330, 100, 40), "Accept")) {
        ...
        //GetComponent.<NetworkView>().RPC ("LoadLevel", RPCMode.AllBuffered, networkPlayerLevel, lastLevelPrefix + 1);
    }
}
    ...
    if (hostData != null && hostData.length > 0) {
        for (var element in hostData) {
            ...
            if (GUILayout.Button ("Join")) {
                ...
                showMenu = 3;
                if(testing){
                	Network.Connect(localIP, element.port);
                }
                else{
